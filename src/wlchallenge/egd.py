""" Copied from: https://github.com/biweidai/PGD/blob/main/PGD.py#L56
"""
...
def PGD(cat, alpha, kl, ks, fpm=None, Omega_m0=0.2726):
    ...
    if fpm is None:
        B = np.ceil(
            2.0 * cat.attrs["BoxSize"] * ks / (2.0 * np.pi * cat.csize ** (1.0 / 3.0))
        )
        mask = B < 1
        B[mask] = 1
        fpm = ParticleMesh(
    ...
    S = (
        alpha
        / H0**2
        * layout.gather(shortrangePM(X1, delta_k, kl, ks, factor=1.5 * Omega_m0))
    )
...
def Gaussian(r_smth):
    def kernel(k, v):
        ...
        return v * np.exp(-kk * r_smth**2 / 2.0)
...
def shortrangePM(x, delta_k, kl, ks, factor):
    ...
    pot_k = delta_k.apply(laplace, out=Ellipsis).apply(shortrange(kl, ks), out=Ellipsis)

    for d in range(x.shape[1]):
        force_d = pot_k.apply(gradient(d)).c2r(out=Ellipsis)
...
def HPM(rho, gamma, x, T0=1e4, muH=1.67372e-27):
    ...
    for d in range(x.shape[1]):
        force_d = enthalpy_k.apply(gradient(d)).c2r(out=Ellipsis)
        force_d.readout(x, out=f[..., d])
