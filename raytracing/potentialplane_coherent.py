...
def main():
    ...
    snapshot_dir = []
    for i in range(12):
        snapshot_dir.append(snapshot_basedir + 'MP-Gadget/' + simulation + '/PART_%03d' % (15-i))
    for i in range(12,24):
        #snapshot_dir.append(snapshot_basedir + 'fastpm_box704/' + simulation + '/Om_%.4f_S8_%.4f_%.4f' % (Omega_m, sigma_8, 1/(1+redshift[i])))
        #snapshot_dir.append(snapshot_basedir + 'fastpm_box704/' + simulation + '_new/Om_%.4f_S8_%.4f_%.4f' % (Omega_m, sigma_8, 1/(1+redshift[i])))
        snapshot_dir.append(snapshot_basedir + 'fastpm_box704/' + simulation + '_new_n4/Om_%.4f_S8_%.4f_%.4f' % (Omega_m, sigma_8, 1/(1+redshift[i])))
    for i in range(24,43):
        snapshot_dir.append(snapshot_basedir + 'fastpm_box1536/' + simulation + '/Om_%.4f_S8_%.4f_%.4f' % (Omega_m, sigma_8, 1/(1+redshift[i])))
    ...
    for snapshot_id in range(params['snapshot_start'],params['snapshot_end']):
        ...
        shift = np.load('shift_coherent.npy')[:,index]
        dim = np.load('dim_coherent.npy')[:,index]
        sign = np.load('sign_coherent.npy')[:,index]
        ...
        if snapshot_id == 0:
            thick = (comoving_dis[0] + comoving_dis[1]) / 2.
        else:
            thick = (comoving_dis[snapshot_id+1] - comoving_dis[snapshot_id-1]) / 2.
        ...
        for realization in range(Nrealization):
            ...
            pos = plane.calculate_potential(part_dir=snapshot_dir[snapshot_id], Nmesh=Nmesh, boxsize=[width,width,thick], los=dim[realization], part_in_kpc=part_in_kpc, shift=shift[realization], pos=pos, return_pos=True, verbose=True)
            
            plane.save(save_dir + 'LensingPlane/' + simulation + '_plane%d_realization%d_coherent' % (snapshot_id, realization), files=['potentialk'])
