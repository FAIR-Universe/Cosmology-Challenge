...
class LensingPlane():
    ...
    def __init__(self, redshift, Omega_m, comoving_distance, fac=None, periodic=True, margin=[0,0], resampler='tsc', memory_efficient=True, comm=MPI.COMM_WORLD):
    ...
    def calculate_potential(self, part_dir, Nmesh, boxsize, los=2, part_in_kpc=False, shift=[0,0,0], flip=(False, False), transpose=False, pos=None, return_pos=False, verbose=True):
        ...
        if verbose and self.comm.rank == 0:
            print('Loading particle at redshift %.2f takes'%self.redshift, time.time()-t, 's', flush=True)
        ...
        if not self.periodic:
            self.margin[0] = boxsize[0] / self.comoving_distance / 4.
            self.margin[1] = boxsize[1] / self.comoving_distance / 4.
        ...
        if return_pos:
            original_pos = pos
        select = (pos[:,x] < boxsize[0]) & (pos[:,y] < boxsize[1]) & (pos[:,los] < boxsize[2])
        pos = pos[select][:,[x,y]]
        ...
        #self.comm.Barrier()
        if verbose and self.comm.rank == 0:
            print('Particle preprocessing at redshift %.2f takes'%self.redshift, time.time()-t, 's', flush=True)
        
        t = time.time()
    
        # paint overdensity
        pm = ParticleMesh(Nmesh=[Nmesh, Nmesh], BoxSize=[boxsize[0]/self.comoving_distance, boxsize[1]/self.comoving_distance], resampler=self.resampler)
        layout = pm.decompose(pos)
        delta = pm.paint(pos, layout=layout, resampler=self.resampler)
        del pos
        delta *= (pm.Nmesh.prod() * part_boxsize**3 / np.prod(boxsize) / Ntotal) # 1 + delta
        delta -= 1
    
        # DensityPlane in lenstools: should be 1.5 * H0^2 / c^2 * Omega_m * comoving_dis * delta_covoming_dis / scale_fac * delta
        c = 299792.458
        H0 = 100
        self.fac = 1.5 * H0 ** 2 / c**2 * self.Omega_m * self.comoving_distance * boxsize[2] * (1+self.redshift) 
        delta *= self.fac 

        #self.comm.Barrier()
        if verbose and self.comm.rank == 0:
            print('Particle painting at redshift %.2f takes'%self.redshift, time.time()-t, 's', flush=True)
        t = time.time()
        ...
        #self.comm.Barrier()
        if verbose and self.comm.rank == 0:
            print('Lensing potential calculation at redshift %.2f takes'%self.redshift, time.time()-t, 's', flush=True)
    ...
    def calculate_deflectionAngles(self, order=1, verbose=True):
        ...
        assert self.potentialk is not None, "Needs to calculate / load lensing potential first!"

        #self.comm.Barrier()
        t = time.time()
        
        self.deflection_x = self.potentialk.apply(gradient(0, order=order)).c2r(out=Ellipsis)
        self.deflection_y = self.potentialk.apply(gradient(1, order=order)).c2r(out=Ellipsis)

        #self.comm.Barrier()
        if verbose and self.comm.rank == 0:
            print('Deflection angle calculation at redshift %.2f takes'%self.redshift, time.time()-t, 's', flush=True)
    ...
    def calculate_shearMatrix(self, verbose=True):
        ...
        if self.potentialk is not None:
            ...
            self.shear_xy = deflection_xk.apply(gradient(1), out=Ellipsis).c2r(out=Ellipsis)
            self.shear_yy = self.deflection_y.r2c().apply(gradient(1), out=Ellipsis).c2r(out=Ellipsis)
        
        #self.comm.Barrier()
        if verbose and self.comm.rank == 0:
            print('Shear matrix calculation at redshift %.2f takes'%self.redshift, time.time()-t, 's', flush=True)
    ...
    def deflectionAngles(self, pos):
        ...
        angle_x = layout.gather(self.deflection_x.readout(pos1, resampler=self.resampler))
        angle_y = layout.gather(self.deflection_y.readout(pos1, resampler=self.resampler))
        if self.flip[1]:
            angle_y *= -1
        if self.flip[0]:
            angle_x *= -1
    ...
    def shearMatrix(self, pos):
        ...
        shear = np.concatenate((shear_xx[None], shear_yy[None], shear_xy[None]), 0) # (3, Npos)
    ...
    def randomRoll(self):
        ...
        if self.periodic:
            self.random_shift = self.comm.bcast(np.random.rand(2) if self.comm.rank==0 else None, root=0)
        
        self.flip = self.comm.bcast(np.random.randn(2)>0 if self.comm.rank==0 else None, root=0)
        self.transpose = self.comm.bcast(np.random.randn()>0 if self.comm.rank==0 else None, root=0)
    ...
    def save(self, filename, files=['potentialk'], verbose=True):
        ...
        #self.comm.Barrier()
        t = time.time()
        assert set(files).issubset({'potentialk', 'deflection', 'shear'}), "Can only save potentialk, deflection and shear"

        # save parameters
        if self.comm.rank == 0:
            if not os.path.exists(filename):
                os.mkdir(filename)
            paramdict = {'redshift': self.redshift,
        ...
        if 'shear' in files:
            assert (self.shear_xx is not None) and (self.shear_xy is not None) and (self.shear_yy is not None)
        ...
        #self.comm.Barrier()
        if verbose and self.comm.rank == 0:
            print('Savinging lensing plane at redshift %.2f to disk takes'%self.redshift, time.time()-t, 's', flush=True)
    ...
    def load(self, filename, files=['potentialk'], verbose=True):
        ...
        #self.comm.Barrier()
        t = time.time()
        assert set(files).issubset({'potentialk', 'deflection', 'shear'}), "Can only load potentialk, deflection and shear"
        ...
        if 'potentialk' in files:
            self.potentialk = BigFileMesh(filename + '/potentialk', dataset='Field', comm=self.comm).to_complex_field()
        
        if 'deflection' in files:
            self.deflection_x = BigFileMesh(filename + '/deflection_x', dataset='Field', comm=self.comm).to_real_field()
            self.deflection_y = BigFileMesh(filename + '/deflection_y', dataset='Field', comm=self.comm).to_real_field()
        
        if 'shear' in files:
            self.shear_xx = BigFileMesh(filename + '/shear_xx', dataset='Field', comm=self.comm).to_real_field()
            self.shear_xy = BigFileMesh(filename + '/shear_xy', dataset='Field', comm=self.comm).to_real_field()
            self.shear_yy = BigFileMesh(filename + '/shear_yy', dataset='Field', comm=self.comm).to_real_field()

        #self.comm.Barrier()
        if verbose and self.comm.rank == 0:
            print('Loading lensing plane at redshift %.2f takes'%self.redshift, time.time()-t, 's', flush=True)
...
class RayTracer():
    ...
    def shoot(self, initial_positions, z, initial_deflection=None, weight_planes=None, save_intermediate=False, IA=0, pz=None, Fz=None, save_Hessian=None, save_final_pos=None, save_Nplane=None, verbose=True):
        ...
        IA=2: IA signal at the position of galaxy
        ...
        if weight_planes is not None:
            ...
            weighted_jacobians = np.zeros((len(weight_planes), 4, initial_positions.shape[1]))
        ...
        if type(z)==np.ndarray:
            ...
            assert z.max()<=2*self.redshift[-1]-self.redshift[-2],"Given the current lenses you can trace up to redshift {0:.2f}!".format(2*self.redshift[-1]-self.redshift[-2])
            ...
            #Check that redshift is not too high given the current lenses
            assert z<2*self.redshift[-1]-self.redshift[-2],"Given the current lenses you can trace up to redshift {0:.2f}!".format(2*self.redshift[-1]-self.redshift[-2])
        ...
        if save_intermediate:
            ...
            if IA == 2:
                all_IA_shear_tensors = np.zeros((last_lens+1, 3, initial_positions.shape[1]))
        ...
        for k in range(last_lens+1):
            ...
            #Log
            if verbose and self.comm.rank == 0:
                print("Crossing lens {0} at redshift z={1:.3f}".format(k,current_lens.redshift), flush=True)
            #self.comm.Barrier()
            start = time.time()
            ...
            now = time.time()
            if verbose and self.comm.rank == 0:
                print("Retrieval of deflection angles completed in {0:.3f}s".format(now-last_timestamp), flush=True)
            last_timestamp = now
            ...
            now = time.time()
            if verbose and self.comm.rank == 0:
                print("Shear matrices retrieved in {0:.3f}s".format(now-last_timestamp), flush=True)
            last_timestamp = now
            ...
            now = time.time()
            if verbose and self.comm.rank == 0:
                print("Geometrical weight factors calculations and deflection scaling completed in {0:.3f}s".format(now-last_timestamp), flush=True)
            last_timestamp = now
            ...
            now = time.time()
            if verbose and self.comm.rank == 0:
                print("Deflection angles computed in {0:.3f}s".format(now-last_timestamp), flush=True)
            last_timestamp = now
            ...
            current_jacobian_deflection += Ck * (np.tensordot(dotter,current_jacobian[:,mask],axes=([2],[0])) * shear_tensors).sum(1)
            
            #self.comm.Barrier()
            now = time.time()
            if verbose and self.comm.rank == 0:
                print("Shear matrix products computed in {0:.3f}s".format(now-last_timestamp), flush=True)
            last_timestamp = now
            ...
            if type(z)==np.ndarray:
                ...
                if IA == 1:
                    ...
                    current_positions[:,last] += current_deflection[:,last[mask]] * (z[None,last] - redshift[k+1]) / (redshift[k+2] - redshift[k+1])
                
                #We need to add the distortions to the jacobians too
                current_jacobian[:,select] += current_jacobian_deflection[:,select[mask]]
                current_jacobian[:,last] += current_jacobian_deflection[:,last[mask]] * (z[None,last] - redshift[k+1]) / (redshift[k+2] - redshift[k+1])
                
                # IA
                if IA == 1:
                    select_pz = (pz[:,0] > redshift[k+1]) & (pz[:,0] < redshift[k+2])
                    Sij = shear_tensors / current_lens.fac / 2.
                    delta = Sij[0] + Sij[1]
                    Sij[:2] -= delta / 3.
                    IA_shear_tensors[0] += Fz(current_lens.redshift) * (Sij[0] - Sij[1]) * np.sum(pz[select_pz, 1])
                    IA_shear_tensors[1] += Fz(current_lens.redshift) * 2 * Sij[2] * np.sum(pz[select_pz, 1])
                    # delta * sij
                    IA_shear_tensors[2] += Fz(current_lens.redshift) * delta * (Sij[0] - Sij[1]) * np.sum(pz[select_pz, 1])
                    IA_shear_tensors[3] += Fz(current_lens.redshift) * 2 * delta * Sij[2] * np.sum(pz[select_pz, 1])
                elif IA == 2:
                    Sij = shear_tensors[:,last[mask]] / current_lens.fac / 2.
                    delta = Sij[0] + Sij[1]
                    Sij[:2] -= delta / 3.
                    IA_shear_tensors[0,last] = Fz(current_lens.redshift) * (Sij[0] - Sij[1])
                ...
                if save_Nplane and k == save_Nplane-1 and save_final_pos:
                    ...
                    if self.comm.rank == 0:
                        ...
                        positions.T.astype(np.float16).tofile(save_final_pos) #(2, Npos)
                        del positions
                    deflection = GatherArray(current_deflection.T, self.comm, root=0)
                    if self.comm.rank == 0:
                        deflection.T.astype(np.float16).tofile(save_final_pos) #(2, Npos)
                        del deflection
                ...
                if save_Hessian and save_Nplane and k < save_Nplane:
                    ...
                    if self.comm.rank == 0:
                        (100.*shear_tensors).T.astype(np.float16).tofile(save_Hessian) #(3, Npos)
                        del shear_tensors
                        if k == save_Nplane - 1:
                            save_Hessian.close()
            ...
            now = time.time()
            if verbose and self.comm.rank == 0:
                print("Addition of deflections completed in {0:.3f}s".format(now-last_timestamp), flush=True)
            last_timestamp = now
            
            if weight_planes is not None:
                weighted_jacobians += weight_planes[:,k].reshape(-1,1,1) * current_jacobian[None]
            ...
            if save_intermediate:
                ...
                if IA == 2:
                    ...
                    delta = Sij[0] + Sij[1]
                    Sij[:2] -= delta / 3.
                    all_IA_shear_tensors[k,0] = Fz(current_lens.redshift) * (Sij[0] - Sij[1])
            ...
            now = time.time()
            if verbose and self.comm.rank == 0:
                print("Lens {0} at z={1:.3f} crossed in {2:.3f}s".format(k,current_lens.redshift,now-start), flush=True)
        
        
        current_positions = current_positions.reshape(2, *shape)
        
        #Return the final positions of the light rays (or jacobians)
        convergence = 1.0 - 0.5*(current_jacobian[0]+current_jacobian[3]).reshape(*shape) 
        shear = np.array([0.5*(current_jacobian[3] - current_jacobian[0]),-0.5*(current_jacobian[1]+current_jacobian[2])]).reshape(2, *shape)
        if IA > 0:
            IA_shear_tensors = IA_shear_tensors.reshape(-1, *shape)
        
        if save_intermediate:
            all_convergence = 1.0 - 0.5*(all_jacobians[:,0]+all_jacobians[:,3]).reshape(-1, *shape)
            all_shear = np.concatenate((0.5*(all_jacobians[:,[3]] - all_jacobians[:,[0]]), -0.5*(all_jacobians[:,[1]]+all_jacobians[:,[2]])), 1).reshape(-1, 2, *shape)
            if IA == 2:
                all_IA_shear_tensors = all_IA_shear_tensors.reshape(last_lens+1, 3, *shape)
                return all_convergence, all_shear, convergence, shear, all_IA_shear_tensors, IA_shear_tensors, current_positions
            elif IA == 1:
                return all_convergence, all_shear, convergence, shear, IA_shear_tensors, current_positions
            else:
                return all_convergence, all_shear, convergence, shear, current_positions
        
        elif weight_planes is not None:
            weighted_convergence = 1.0 - 0.5*(weighted_jacobians[:,0]+weighted_jacobians[:,3]).reshape(-1, *shape)
            weighted_shear = np.concatenate((0.5*(weighted_jacobians[:,[3]] - weighted_jacobians[:,[0]]), -0.5*(weighted_jacobians[:,[1]]+weighted_jacobians[:,[2]])), 1).reshape(-1, 2, *shape)
            if IA == 0:
                return weighted_convergence, weighted_shear, convergence, shear, current_positions
            else:
                return weighted_convergence, weighted_shear, convergence, shear, IA_shear_tensors, current_positions
...
def shear_catalog(galaxy_positions, z, shearMatrix, Nmesh, FoV, plane_redshifts, plane_distance, Omega_m, IA=0, pz=None, Fz=None, resampler='SYM20'):
    ...
    #Check that redshift is not too high given the current lenses
    assert z.max()<=2*plane_redshifts[-1]-plane_redshifts[-2],"Given the current lenses you can trace up to redshift {0:.2f}!".format(2*plane_redshifts[-1]-plane_redshifts[-2])
    ...
    for k in range(last_lens+1):
        ...
        if k < len(shearMatrix):
            ...
            #This is the part in which the products with the shear matrix are computed
            current_jacobian_deflection[0] += Ck * (current_jacobian[0] * shear_tensors[0] + current_jacobian[2] * shear_tensors[2])
            current_jacobian_deflection[1] += Ck * (current_jacobian[1] * shear_tensors[0] + current_jacobian[3] * shear_tensors[2])
            current_jacobian_deflection[2] += Ck * (current_jacobian[2] * shear_tensors[1] + current_jacobian[0] * shear_tensors[2])
            current_jacobian_deflection[3] += Ck * (current_jacobian[3] * shear_tensors[1] + current_jacobian[1] * shear_tensors[2])
        ...
        if last.any():
            ...
            field[:] = (current_jacobian[1] + current_jacobian[2]).reshape(Nmesh, Nmesh)/ 2.
            ...
            field[:] = (current_jacobian_deflection[1] + current_jacobian_deflection[2]).reshape(Nmesh, Nmesh) / 2.
        ...
        if k < len(shearMatrix) and IA > 0 and last.any():
            ...
            if IA == 1:
                select_pz = (pz[:,0] > redshift[k+1]) & (pz[:,0] < redshift[k+2])
                IA_shear_tensors[0] += Fz(redshift[k+1]) * (Sij[0] - Sij[1]) * np.sum(pz[select_pz, 1])
                IA_shear_tensors[1] += Fz(redshift[k+1]) * 2 * Sij[2] * np.sum(pz[select_pz, 1])
                # delta * sij
                IA_shear_tensors[2] += Fz(redshift[k+1]) * delta * (Sij[0] - Sij[1]) * np.sum(pz[select_pz, 1])
                IA_shear_tensors[3] += Fz(redshift[k+1]) * 2 * delta * Sij[2] * np.sum(pz[select_pz, 1])
            elif IA == 2:
                field[:] = (Sij[0] - Sij[1]).reshape(Nmesh, Nmesh)
                IA_shear_tensors[0,last] = Fz(redshift[k+1]) * field.readout(galaxy_positions[last])
                field[:] = (2 * Sij[2]).reshape(Nmesh, Nmesh)
                IA_shear_tensors[1,last] = Fz(redshift[k+1]) * field.readout(galaxy_positions[last])
...
def IA_convergence(shearMatrix, plane_redshifts, plane_distance, Omega_m, pz, Fz):
    ...
    for k in range(len(shearMatrix)):
        ...
        if k == 0:
            thick = (distance[1] + distance[2]) / 2.
            z1 = 0
            z2 = (redshift[k+1] + redshift[k+2]) / 2.
        else:
            thick = (distance[k+2] - distance[k]) / 2.
            z1 = (redshift[k] + redshift[k+1]) / 2.
            z2 = (redshift[k+1] + redshift[k+2]) / 2.
...
def save_convergence_planes(shearMatrix, plane_redshifts, plane_distance, Omega_m, Fz, save_dir):
    ...
    for k in range(len(plane_redshifts)):
        ...
        convergence = 1.0 - 0.5*(current_jacobian[0]+current_jacobian[3]).reshape(Nmesh, Nmesh) # plane_redshifts[k+1]
